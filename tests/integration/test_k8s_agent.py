...
async def test_jenkins_k8s_agent_relation(
    ...
    assert not any(jenkins_k8s_agents.name in key for key in jenkins_client.get_nodes())
    assert not any(extra_jenkins_k8s_agents.name in key for key in jenkins_client.get_nodes())
